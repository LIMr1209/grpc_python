...
class ClientStream:
    ...
    def __init__(self):
        ...
        self.sub = async_pb2_grpc.StreamRpcStub(channel=self.channel)

    async def GetServerResult(self):
        # 异步调用rpc
        result = await self.sub.GetServerResult(async_pb2.Requests(data="aaa"))
        print("GetServerResult result: ", result)

    # 单向流
    async def GetServerStream(self):
        # 获取单向流对象
        stream_client = self.sub.GetServerStream(async_pb2.Requests(data="aaa"))
        # 通过异步for 循环获取响应数据
    ...
    async def ClientSendStream(self):
        ...
        for i in range(10):
            await stream_client.write(async_pb2.Requests(data="{}".format(i)))
    ...
    async def ClientSendStreamServer(self, stream_client):
        for i in range(6):
            ...
            await stream_client.write(async_pb2.Requests(data="send data {}".format(i)))
